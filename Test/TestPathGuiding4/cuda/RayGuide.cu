...
extern "C" __global__ void __raygen__pg_def() {
    ...
    do {
        ...
        if(!params.isFinal){
            if (params.isBuilt) {
                for (int j = 0; j < depth; ++j) {
                    vertices[j].Commit<RAY_TRACE_S_FILTER, RAY_TRACE_D_FILTER, true>(params.sdTree, 1.0f);
                }
            }
            else {
                for (int j = 0; j < depth; ++j) {
                    vertices[j].Commit<RAY_TRACE_S_FILTER, RAY_TRACE_D_FILTER, false>(params.sdTree, 1.0f);
                }
            }
...
extern "C" __global__ void __closesthit__radiance_for_diffuse_nee() {
...

    {
        ...
        prd->radiance += lRec.emission * prvThroughPut * static_cast<float>(params.light.count) * weight * diffuse / RTLIB_M_PI;
    }
...
extern "C" __global__ void __closesthit__radiance_for_phong_pg_nee_with_nee_light() {
    ...
    if (rnd1 < a_diffuse) {
        ...
        }
        {
            ...
            prd->radiance += lRec.emission * prvThroughPut * static_cast<float>(params.light.count) * weight * diffuseLobe;
        }
        prd->countEmitted = false;
    }
    else if (rnd1 < a_diffuse + a_specular)
