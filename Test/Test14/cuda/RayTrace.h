...
struct HitgroupData{
    ...
    float               shinness;
#ifdef __CUDACC__
    RTLIB_HOST_DEVICE float3 getDiffuseColor(const float2& uv)const noexcept{ 
    ...
    RTLIB_HOST_DEVICE float3 getSpecularColor(const float2& uv)const noexcept {
    #if defined(TEST_SKIP_TEXTURE_SAMPLE)
    ...
    RTLIB_HOST_DEVICE float3 getEmissionColor(const float2& uv)const noexcept {
    #if defined(TEST_SKIP_TEXTURE_SAMPLE)
