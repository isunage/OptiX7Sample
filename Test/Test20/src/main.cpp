...
public:
	...
	void LoadScene() {
		{
			...
			if (!objMeshGroup->Load(TEST_TEST20_DATA_PATH"/Models/CornellBox/CornellBox-Original.obj", TEST_TEST20_DATA_PATH"/Models/CornellBox/")) {
				throw std::runtime_error("Failed To Load Model!");
			}
			m_MaterialSet = objMeshGroup->GetMaterialList();

		m_Tracer.AddMeshGroup("CornellBox-Water", objMeshGroup->GetMeshGroup());
			{
		...
		auto tlasHandle = m_Tracer2.GetIASHandle("TLAS");
		{
			...
			instanceSet->instanceBuffer.Upload();
	...
	void InitTracePipeline()
	{
		...
		//SBTRecord
		{
			...
			{
				tracePipeline2->NewHitGRecordBuffer("Mis", RAY_TYPE_COUNT * m_Tracer2.GetTLAS()->GetSbtCount());
				{
					size_t sbtOffset = 0;
					for (auto& instanceSet : m_Tracer2.GetTLAS()->GetInstanceSets()) {
						for (auto& baseGASHandle : instanceSet->baseGASHandles) {
	...
	void InitStorePipeline()
	{
		...
		//SBTRecord
		{
			...
			{
				storePipeline2->NewHitGRecordBuffer("Def", RAY_TYPE_COUNT * m_Tracer2.GetTLAS()->GetSbtCount());
				{
					size_t sbtOffset = 0;
					for (auto& instanceSet : m_Tracer2.GetTLAS()->GetInstanceSets()) {
						for (auto& baseGASHandle : instanceSet->baseGASHandles) {
