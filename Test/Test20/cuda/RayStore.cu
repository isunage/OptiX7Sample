...
extern "C" {
    __constant__ RayStoreParams params;
}
...
extern "C" __global__ void __raygen__rg(){
    ...
    RadianceRec radianceRecs[MAX_TRACE_DEPTH];
    do {
        ...
        for (;;) {
            ...
            radianceRecs[depth].origin    = rayOrigin;
            radianceRecs[depth].direction = rayDirection;
            if (prd.done ) {
                radianceRecs[depth].data  = prd.emitted;
                isValid = true;
                break;
            }else if( depth >= params.maxTraceDepth){
                break;
            }else{
                radianceRecs[depth].data = prd.attenuation;
            }
            depth++;
        }
        seed = prd.seed;
        if (isValid){
            unsigned int curRecCount = atomicAdd(params.radianceRecCount,depth);
            for(int j=0;j<depth;++j){
                params.radianceRecData[curRecCount+j].origin    = radianceRecs[j].origin;
                params.radianceRecData[curRecCount+j].direction = radianceRecs[j].direction;
...
extern "C" __global__ void __miss__radiance(){
    ...
    prd->emitted     = make_float3(msData->bgColor.x, msData->bgColor.y, msData->bgColor.z);
