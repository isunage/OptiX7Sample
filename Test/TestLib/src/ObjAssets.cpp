#include "../include/TestLib/Assets/ObjAssets.h"
#include <filesystem>
#include <unordered_set>
#include <tiny_obj_loader.h>
bool test::assets::ObjAsset::Load(const rtlib::ext::VariableMap& params)
{
    if (m_Valid)
    {
        Free();
    }
	auto objFilePath = params.GetString("objFilePath");
	auto mtlFileDir  = params.GetString("mtlFileDir");
    auto mtlBaseDir  = std::filesystem::canonical(mtlFileDir);
    ...
    }
    {
        ...
        {
            meshGroup->RemoveMesh(name);
            auto newEmitUniqueResources = std::vector< rtlib::ext::MeshUniqueResourcePtr>(uniqueResource->materials.size());
            ...
            for (auto i = 0; i < uniqueResource->matIndBuffer.Size(); ++i) {
                auto  matIndex = uniqueResource->matIndBuffer[i];
                newEmitUniqueResources[matIndex]->matIndBuffer.PushBack(0);
                newEmitUniqueResources[matIndex]->triIndBuffer.PushBack(uniqueResource->triIndBuffer[i]);
            }
            for (auto i = 0; i < uniqueResource->materials.size()   ; ++i)
            {
    ...
    m_Valid     = true;
    m_Materials = std::move(phongMaterials);
    m_MeshGroup = std::move(meshGroup);
	return true;
}

void test::assets::ObjAsset::Free()
{
    m_Valid = false;
    if (!m_MeshGroup) {
        return;
    }
    if (m_MeshGroup->GetSharedResource()) {
        m_MeshGroup->GetSharedResource()->vertexBuffer.Reset();
        m_MeshGroup->GetSharedResource()->texCrdBuffer.Reset();
        m_MeshGroup->GetSharedResource()->normalBuffer.Reset();
    }
    for (auto& [name, uniqueResource] : m_MeshGroup->GetUniqueResources())
    {
        if (uniqueResource) {
            uniqueResource->matIndBuffer.Reset();
            uniqueResource->triIndBuffer.Reset();
        }
    }
}

bool test::assets::ObjAsset::IsValid() const
{
	return m_Valid;
}

bool test::assets::ObjAssetManager::LoadAsset(const std::string& key, const rtlib::ext::VariableMap& params)
{
    auto asset = test::assets::ObjAsset::New();
    bool cond  = asset->Load(params);
    if (cond) {
        GetAssets()[key] = asset;
        asset->GetMeshGroup()->GetSharedResource()->variables.SetString("objAssetName", key);
    }
    return cond;
}

void test::assets::ObjAssetManager::FreeAsset(const std::string& key)
{
    if (HasAsset(key))
    {
