#include "../include/TestLib/RTPathGuidingUtils.h"

test::RTDTree::RTDTree() noexcept {
	m_MaxDepth = 0;
...
void test::RTDTree::Reset(const RTDTree& prvDTree, int newMaxDepth, float subDivTh) {
	m_Area = 0.0f;
	m_Sum = 0.0f;
	m_StatisticalWeight = 0.0f;
	m_MaxDepth = 0;
...
auto test::RTDTree::GetSum() const noexcept -> float {
	return m_Sum;
}

void test::RTDTree::SetSum(float val) noexcept {
	m_Sum = val;
}

void test::RTDTree::SetStatisticalWeight(float val) noexcept {
	m_StatisticalWeight = val;
}

auto test::RTDTree::GetStatisticalWeight() const noexcept -> float {
	return m_StatisticalWeight;
}

auto test::RTDTree::GetApproxMemoryFootPrint() const noexcept -> size_t {
	return sizeof(DTreeNode) * m_Nodes.size() + sizeof(DTree);
}

auto test::RTDTree::GetNumNodes() const noexcept -> size_t {
	return m_Nodes.size();
}

auto test::RTDTree::GetDepth() const noexcept -> int {
	return m_MaxDepth;
}

auto test::RTDTree::GetMean() const noexcept -> float {
	if (m_StatisticalWeight * m_Area <= 0.0f) { return 0.0f; }
	const float factor = 1.0f / (4.0f * RTLIB_M_PI * m_Area * m_StatisticalWeight);
	return factor * m_Sum;
}

auto test::RTDTree::GetArea() const noexcept -> float {
	return m_Area;
}

auto test::RTDTree::Pdf(const float3& dir) const noexcept -> float {
	if (GetMean() <= 0.0f) {
...
void test::RTDTree::Dump(std::fstream& jsonFile) const noexcept {
	jsonFile << "{\n";
	jsonFile << "\"sum\"              : " << m_Sum << ",\n";
...
auto test::RTDTree::Nodes() noexcept -> std::vector<RTDTreeNode>& {
	return m_Nodes;
}

auto test::RTDTree::Node(size_t idx) const noexcept -> const RTDTreeNode& {
	return m_Nodes[idx];
}

auto test::RTDTree::Node(size_t idx) noexcept -> RTDTreeNode& {
	return m_Nodes[idx];
}

auto test::RTDTreeWrapper::GetApproxMemoryFootPrint() const noexcept -> size_t {
	return building.GetApproxMemoryFootPrint() + sampling.GetApproxMemoryFootPrint();
}

auto test::RTDTreeWrapper::GetStatisticalWeightSampling() const noexcept -> float {
	return sampling.GetStatisticalWeight();
}

auto test::RTDTreeWrapper::GetStatisticalWeightBuilding() const noexcept -> float {
	return building.GetStatisticalWeight();
}

void test::RTDTreeWrapper::SetStatisticalWeightSampling(float val) noexcept {
	sampling.SetStatisticalWeight(val);
}

void test::RTDTreeWrapper::SetStatisticalWeightBuilding(float val) noexcept {
	building.SetStatisticalWeight(val);
}

auto test::RTDTreeWrapper::Pdf(const float3& dir) const noexcept -> float {
	return sampling.Pdf(dir);
}

auto test::RTDTreeWrapper::GetNumNodes() const noexcept -> size_t {
	return sampling.GetNumNodes();
}

auto test::RTDTreeWrapper::GetMean() const noexcept -> float {
	return sampling.GetMean();
}

auto test::RTDTreeWrapper::GetArea() const noexcept -> float {
	return sampling.GetArea();
}

auto test::RTDTreeWrapper::GetDepth() const noexcept -> int {
	return sampling.GetDepth();
}

void test::RTDTreeWrapper::Build() {
	//一層にする→うまくいく
	building.Build();
	sampling = building;
}

auto test::RTSTreeNode::GetChildIdx(float3& p) const noexcept -> int {
...
auto test::RTSTreeNode::GetNodeIdx(float3& p) const noexcept -> unsigned int {
	return children[GetChildIdx(p)];
}

auto test::RTSTreeNode::GetDTree(float3 p, float3& size, const std::vector<RTSTreeNode>& nodes) const noexcept -> const RTDTreeWrapper* {
...
auto test::RTSTreeNode::GetDTreeWrapper() const noexcept -> const RTDTreeWrapper* {
	return &dTree;
}

auto test::RTSTreeNode::GetDepth(const std::vector<RTSTreeNode>& nodes) const -> int {
	int result = 1;
	if (isLeaf) {
		return 1;
	}
...
void test::RTSTreeNode::Dump(std::fstream& jsonFile, size_t sTreeNodeIdx, const std::vector<RTSTreeNode>& nodes) const noexcept {
	jsonFile << "{\n";
...
void test::RTSTree::Clear() noexcept {
	m_Nodes.clear();
	m_Nodes.emplace_back();
}

void test::RTSTree::SubDivideAll() {
...
void test::RTSTree::SubDivide(int nodeIdx, std::vector<RTSTreeNode>& nodes)
{
...
auto test::RTSTree::GetDTree(float3 p, float3& size) const noexcept -> const RTDTreeWrapper* {
...
auto test::RTSTree::GetDTree(const float3& p) const noexcept -> const RTDTreeWrapper* {
	float3 size;
	return GetDTree(p, size);
}

auto test::RTSTree::GetDepth() const -> int {
	return m_Nodes[0].GetDepth(m_Nodes);
}

auto test::RTSTree::Node(size_t idx) const noexcept -> const RTSTreeNode& {
	return m_Nodes[idx];
}

auto test::RTSTree::Node(size_t idx) noexcept -> RTSTreeNode& {
	return m_Nodes[idx];
}

auto test::RTSTree::GetNumNodes() const noexcept -> size_t {
	return m_Nodes.size();
}

bool test::RTSTree::ShallSplit(const RTSTreeNode& node, int depth, size_t samplesRequired) const noexcept
{
...
void test::RTSTree::Refine(size_t sTreeTh, int maxMB) {
	if (maxMB >= 0) {
...
auto test::RTSTree::GetAabbMin() const -> float3 {
	return m_AabbMin;
}

auto test::RTSTree::GetAabbMax() const -> float3 {
	return m_AabbMax;
}

void test::RTSTree::Dump(std::fstream& jsonFile) const noexcept {
	jsonFile << "{\n";
...
void test::RTSTreeWrapper::Upload() noexcept {
	//Uploadは両方必要
...
void test::RTSTreeWrapper::Download() noexcept {
...
void test::RTSTreeWrapper::Clear() {
	m_CpuSTree = RTSTree(m_CpuSTree.GetAabbMin(), m_CpuSTree.GetAabbMax());
}

auto test::RTSTreeWrapper::GetGpuHandle() const noexcept -> STree {
	STree sTree;
...
void test::RTSTreeWrapper::Reset(int iter, int samplePerPasses) {
	if (iter <= 0) {
		return;
	}
...
void test::RTSTreeWrapper::Build() {
	size_t bestIdx = 0;
	for (int i = 0; i < m_CpuSTree.GetNumNodes(); ++i) {
		if (m_CpuSTree.Node(i).isLeaf) {
...
void test::RTSTreeWrapper::Dump(std::string filename) {
