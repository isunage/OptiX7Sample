...
bool test::RTTextureAssetManager::LoadAsset(const std::string& keyName, const std::string& texPath)
{
    ...
    {
        auto img = test::LoadImgFromPathAsUcharData(texPath, texWidth, texHeight, texComp, 4);
        if (!img) {
            return false;
        }
...
auto  test::RTTextureAssetManager::GetAssets()const -> const std::unordered_map<std::string, rtlib::ext::CustomImage2D<uchar4>>& {
    return m_Textures;
}
auto  test::RTTextureAssetManager::GetAssets()->std::unordered_map<std::string, rtlib::ext::CustomImage2D<uchar4>>& {
    return m_Textures;
}
...
test::RTTextureAssetManager::~RTTextureAssetManager() {
    Reset();
}
