...
namespace test {
	...
	public:
		...
		auto  GetAsset(const std::string& keyName)const -> const rtlib::ext::CustomImage2D<uchar4>&;
		auto  GetAsset(const std::string& keyName)      ->       rtlib::ext::CustomImage2D<uchar4>&;
		auto  GetAssets()const -> const std::unordered_map<std::string, rtlib::ext::CustomImage2D<uchar4>>&;
		auto  GetAssets()      ->       std::unordered_map<std::string, rtlib::ext::CustomImage2D<uchar4>>&;
		bool  HasAsset(const std::string& keyName)const noexcept;
		void Reset();
		~RTTextureAssetManager();
	private:
		std::unordered_map<std::string, rtlib::ext::CustomImage2D<uchar4>> m_Textures = {};
	};
	struct RTObjModel {
