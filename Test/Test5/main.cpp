...
int main(){
    ...
    try{
        ...
        auto    d_RaygenBuffer = rtlib::CUDABuffer<decltype(  raygenRecord)>();
        d_RaygenBuffer.allocate(1);
        d_RaygenBuffer.upload(&raygenRecord, 1);
        auto      d_MissBuffer = rtlib::CUDABuffer<decltype(    missRecord)>();
        d_MissBuffer.allocate(1);
        d_MissBuffer.upload(&missRecord, 1);
        auto  d_HitgroupBuffer = rtlib::CUDABuffer<decltype(hitgroupRecord)>();
        d_HitgroupBuffer.allocate(1);
        d_HitgroupBuffer.upload(&hitgroupRecord, 1);
