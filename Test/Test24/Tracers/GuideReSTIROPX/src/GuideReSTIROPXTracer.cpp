#include "..\include\GuideReSTIROPXTracer.h"
...
struct Test24GuideReSTIROPXTracer::Impl
{
	...
	Impl(
		...
		const unsigned int& maxTraceDepth) :m_Context{ Context }, m_SharedFramebuffer{ Framebuffer }, m_CameraController{ CameraController }, m_TextureManager{ TextureManager }, m_Materials{ Materials }, m_TopLevelAS{ TopLevelAS }, m_BgLightColor{ BgLightColor },m_CurEventFlags{ EventFlags }, m_PrvEventFlags{ TEST24_EVENT_FLAG_NONE },m_MaxTraceDepth{maxTraceDepth}
	{
	}
	~Impl() {
	}
...
Test24GuideReSTIROPXTracer::Test24GuideReSTIROPXTracer(
...
{
	m_Impl = std::make_unique<Test24GuideReSTIROPXTracer::Impl>(
		Context, Framebuffer, CameraController, TextureManager, TopLevelAS, Materials, BgLightColor,eventFlags, maxTraceDepth
		);
	//MOVABLE
...
void Test24GuideReSTIROPXTracer::Initialize()
{
	this->InitPipeline();
...
void Test24GuideReSTIROPXTracer::Launch(int width, int height, void* userData)
{
...
void Test24GuideReSTIROPXTracer::Update()
{
...
void Test24GuideReSTIROPXTracer::InitFrameResources()
{
...
void Test24GuideReSTIROPXTracer::InitPipeline()
{
	/**First **/
	{
		auto rayReSTIRPtxFile = std::ifstream(TEST_TEST24_GUIDE_RESTIR_OPX_CUDA_PATH "/RayFirst.ptx", std::ios::binary);
	...
	/**Second**/
	{
		auto rayReSTIRPtxFile = std::ifstream(TEST_TEST24_GUIDE_RESTIR_OPX_CUDA_PATH "/RaySecond.ptx", std::ios::binary);
		if (!rayReSTIRPtxFile.is_open())
	...
	{
		auto rayReSTIRPtxFile = std::ifstream(TEST_TEST24_GUIDE_RESTIR_OPX_CUDA_PATH "/ReservoirReuse.ptx", std::ios::binary);
		if (!rayReSTIRPtxFile.is_open())
			throw std::runtime_error("Failed To Load RaySecond.ptx!");
		auto rayReSTIRPtxData = std::string((std::istreambuf_iterator<char>(rayReSTIRPtxFile)), (std::istreambuf_iterator<char>()));
		rayReSTIRPtxFile.close(); 
...
void Test24GuideReSTIROPXTracer::InitShaderBindingTable()
{
	/**First **/
	{
...
void Test24GuideReSTIROPXTracer::InitLaunchParams()
{
	auto samplePerAll = GetVariables()->GetUInt32("SamplePerAll");
	auto samplePerLaunch = GetVariables()->GetUInt32("SamplePerLaunch");
...
void Test24GuideReSTIROPXTracer::FreeFrameResources()
{
	this->m_Impl->m_UniqueFramebuffer.reset();
}

void Test24GuideReSTIROPXTracer::FreePipeline()
{
...
void Test24GuideReSTIROPXTracer::FreeShaderBindingTable()
{
...
void Test24GuideReSTIROPXTracer::FreeLight()
{

}

void Test24GuideReSTIROPXTracer::FreeLaunchParams()
{
