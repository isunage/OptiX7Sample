...
extern "C" __global__ void __closesthit__radiance_for_diffuse_nee_with_nee_light(){
    ...
    }
    {
        ...
        if (ndl > 0.0f && lRec.invPdf > 0.0f) {
            const bool occluded = traceOccluded(params.gasHandle, position + 0.01f * normal, lightDir, 0.0f, lRec.distance - 0.01f);
            if (!occluded) {
    ...
    rtlib::Xorshift32 xor32(prd->seed);
    {
        ...
        if (rnd < a_diffuse) {
            ...
            prd->countEmitted = false;
            {
                ...
                float weight2  = 0.0f;
                if (ndl > 0.0f && lRec.invPdf > 0.0f) {
                    const bool occluded = traceOccluded(params.gasHandle, position, lightDir, 0.001f, lRec.distance - 0.001f);
                    if (!occluded) {
