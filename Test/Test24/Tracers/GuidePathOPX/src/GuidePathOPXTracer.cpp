...
struct Test24GuidePathOPXTracer::Impl
{
    ...
    std::shared_ptr< test::RTSTreeWrapper> m_STree = nullptr;
...
void Test24GuidePathOPXTracer::Launch(int width, int height, void* pUserData)
{
    UserData* userData = (UserData*)pUserData;
    OnLaunchBegin(width,height,userData);
    OnLaunchExecute(width, height, userData);
    OnLaunchEnd(width, height, userData);
}
...
bool Test24GuidePathOPXTracer::ShouldLock() const noexcept { return true; }
...
void Test24GuidePathOPXTracer::InitSdTree()
{
    ...
    for (auto& mesh : m_Impl->m_TopLevelAS->GetInstanceSets()[0]->GetInstance(0).baseGASHandle->GetMeshes())
    {
    ...
    m_Impl->m_STree = std::make_shared<test::RTSTreeWrapper>(worldAABB.min, worldAABB.max, 20);
...
void Test24GuidePathOPXTracer::InitPipeline()
{
    ...
    this->m_Impl->m_Pipeline = this->m_Impl->m_Context->GetOPX7Handle()->createPipeline(guideCompileOptions);
    this->m_Impl->m_Modules["RayGuide"] = this->m_Impl->m_Pipeline.createModule(rayGuidePtxData, guideModuleOptions);
    this->m_Impl->m_RGProgramGroups["Guide.Default"] = this->m_Impl->m_Pipeline.createRaygenPG({ this->m_Impl->m_Modules["RayGuide"], RTLIB_RAYGEN_PROGRAM_STR(def) });
    this->m_Impl->m_RGProgramGroups["Guide.Guiding.Default"] = this->m_Impl->m_Pipeline.createRaygenPG({ this->m_Impl->m_Modules["RayGuide"], RTLIB_RAYGEN_PROGRAM_STR(pg_def) });
    this->m_Impl->m_MSProgramGroups["Guide.Radiance"] = this->m_Impl->m_Pipeline.createMissPG({ this->m_Impl->m_Modules["RayGuide"], RTLIB_MISS_PROGRAM_STR(radiance) });
    this->m_Impl->m_MSProgramGroups["Guide.Occluded"] = this->m_Impl->m_Pipeline.createMissPG({ this->m_Impl->m_Modules["RayGuide"], RTLIB_MISS_PROGRAM_STR(occluded) });
    this->m_Impl->m_HGProgramGroups["Guide.Radiance.Diffuse.Guiding.Default"] = this->m_Impl->m_Pipeline.createHitgroupPG({ this->m_Impl->m_Modules["RayGuide"], RTLIB_CLOSESTHIT_PROGRAM_STR(radiance_for_diffuse_pg_def) }, {}, {});
    this->m_Impl->m_HGProgramGroups["Guide.Radiance.Phong.Guiding.Default"] = this->m_Impl->m_Pipeline.createHitgroupPG({ this->m_Impl->m_Modules["RayGuide"], RTLIB_CLOSESTHIT_PROGRAM_STR(radiance_for_phong_pg_def) }, {}, {});
    this->m_Impl->m_HGProgramGroups["Guide.Radiance.Emission"] = this->m_Impl->m_Pipeline.createHitgroupPG({ this->m_Impl->m_Modules["RayGuide"], RTLIB_CLOSESTHIT_PROGRAM_STR(radiance_for_emission) }, {}, {});
    this->m_Impl->m_HGProgramGroups["Guide.Radiance.Specular"] = this->m_Impl->m_Pipeline.createHitgroupPG({ this->m_Impl->m_Modules["RayGuide"], RTLIB_CLOSESTHIT_PROGRAM_STR(radiance_for_specular) }, {}, {});
    this->m_Impl->m_HGProgramGroups["Guide.Radiance.Refraction"] = this->m_Impl->m_Pipeline.createHitgroupPG({ this->m_Impl->m_Modules["RayGuide"], RTLIB_CLOSESTHIT_PROGRAM_STR(radiance_for_refraction) }, {}, {});
    this->m_Impl->m_HGProgramGroups["Guide.Occluded"] = this->m_Impl->m_Pipeline.createHitgroupPG({ this->m_Impl->m_Modules["RayGuide"], RTLIB_CLOSESTHIT_PROGRAM_STR(occluded) }, {}, {});
    this->m_Impl->m_Pipeline.link(guideLinkOptions);
}
...
void Test24GuidePathOPXTracer::OnLaunchBegin(int width, int height,  UserData* config)
{
    if (config->samplePerAll == 0)
    {
        this->m_Impl->m_SamplePerAll = 0;
        this->m_Impl->m_SamplePerTmp = 0;
        this->m_Impl->m_SampleForBudget = config->sampleForBudget;
        this->m_Impl->m_SamplePerLaunch = config->samplePerLaunch;
...
void Test24GuidePathOPXTracer::OnLaunchExecute(int width, int height,  UserData* config)
{
    ...
    this->m_Impl->m_Params.cpuHandle[0].seedBuffer = this->m_Impl->m_SeedBuffer.getDevicePtr();
    this->m_Impl->m_Params.cpuHandle[0].isBuilt = this->m_Impl->m_CurIteration  > config->iterationForBuilt;
    ...
    cudaMemcpyAsync(this->m_Impl->m_Params.gpuHandle.getDevicePtr(), &this->m_Impl->m_Params.cpuHandle[0], sizeof(RayTraceParams), cudaMemcpyHostToDevice, config->stream);
    ...
    this->m_Impl->m_Pipeline.launch(config->stream, this->m_Impl->m_Params.gpuHandle.getDevicePtr(), this->m_Impl->m_ShaderBindingTable, (unsigned)width, (unsigned)height, 1);
    if (config->isSync)
    {
    ...
    this->m_Impl->m_SamplePerAll = config->samplePerAll = this->m_Impl->m_Params.cpuHandle[0].samplePerALL;
    this->m_Impl->m_SamplePerTmp += this->m_Impl->m_SamplePerLaunch;
}

void Test24GuidePathOPXTracer::OnLaunchEnd(int width, int height,  UserData* config)
{
