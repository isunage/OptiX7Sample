#include "../include/CallableOPXTracer.h"
#include <RayTrace.h>
#include <fstream>
#include <random>
using namespace std::string_literals;
using namespace test24_callable;
...
// RTTracer ����Čp������܂���
struct Test24CallableOPXTracer::Impl {
	Impl(
		ContextPtr context,
...
Test24CallableOPXTracer::Test24CallableOPXTracer(
	ContextPtr context, 
	FramebufferPtr framebuffer, 
	CameraControllerPtr cameraController, 
	TextureAssetManager textureManager, 
	...
	const unsigned int& eventFlags,
	const unsigned int& maxTraceDepth){
	m_Impl = std::make_unique<Test24CallableOPXTracer::Impl>(
		context,
		framebuffer,
		cameraController,
		textureManager,
...
void Test24CallableOPXTracer::Initialize()
{
 ...
 void Test24CallableOPXTracer::Launch(int width, int height, void* pdata)
{
 ...
 void Test24CallableOPXTracer::CleanUp()
{
	this->FreePipeline();
 ...
 void Test24CallableOPXTracer::Update()
{
 ...
 void Test24CallableOPXTracer::InitPipeline()
{
	auto rayTracePtxFile = std::ifstream(TEST_TEST24_CALLABLE_OPX_CUDA_PATH "/RayTrace.ptx", std::ios::binary);
 ...
 void Test24CallableOPXTracer::InitFrameResources()
{
	std::vector<unsigned int> seedData(this->m_Impl->m_Framebuffer->GetWidth() * this->m_Impl->m_Framebuffer->GetHeight());
	std::random_device rd;
	std::mt19937 mt(rd());
 ...
 void Test24CallableOPXTracer::InitShaderBindingTable()
{
	float aspect = static_cast<float>(m_Impl->m_Framebuffer->GetWidth()) / static_cast<float>(m_Impl->m_Framebuffer->GetHeight());
 ...
 void Test24CallableOPXTracer::InitLaunchParams()
{
	auto tlas = this->m_Impl->m_TopLevelAS;
 ...
 void Test24CallableOPXTracer::FreeLight()
 {
 }

 void Test24CallableOPXTracer::FreePipeline()
{
 ...
 void Test24CallableOPXTracer::FreeShaderBindingTable()
{
 ...
 void Test24CallableOPXTracer::FreeLaunchParams()
{
	this->m_Impl->m_Params.Reset();
}

 void Test24CallableOPXTracer::FreeFrameResources()
{
