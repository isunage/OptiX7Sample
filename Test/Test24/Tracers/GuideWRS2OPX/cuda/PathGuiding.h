...
namespace test24_wrs2_guide
{
	...
	struct DTreeNode2
	{
		RTLIB_INLINE RTLIB_HOST_DEVICE DTreeNode2() noexcept
		{
			for (int i = 0; i < 4; ++i)
			{
				children[i] = 0;
		...
		RTLIB_INLINE RTLIB_HOST_DEVICE DTreeNode2(const DTreeNode2& node) noexcept
		{
			CopyFrom(node);
		}
		RTLIB_INLINE RTLIB_HOST_DEVICE DTreeNode2& operator=(const DTreeNode2& node) noexcept
		{
			CopyFrom(node);
			return *this;
		}
		RTLIB_INLINE RTLIB_HOST_DEVICE void CopyFrom(const DTreeNode2& node) noexcept
		{
			for (int i = 0; i < 4; ++i)
			{
				sums[i] = node.sums[i];
		...
		//VAL
		RTLIB_INLINE RTLIB_HOST_DEVICE auto GetVal(int childIdx) const noexcept-> float
		{
			return weis[childIdx] > 0.0f ? sums[childIdx] / weis[childIdx] :0.0f;
		}
		//WEI
		RTLIB_INLINE RTLIB_HOST_DEVICE auto GetWei(int childIdx) const noexcept -> float
		{
			return weis[childIdx];
		}
		RTLIB_INLINE RTLIB_HOST_DEVICE void SetWei(int childIdx, float val) noexcept
		{
			weis[childIdx] = val;
		}
		RTLIB_INLINE RTLIB_HOST_DEVICE void SetWeiAll(float val) noexcept
		{
			for (int i = 0; i < 4; ++i)
			{
				weis[i] = val;
		...
		RTLIB_INLINE RTLIB_HOST_DEVICE auto GetWeiOfAll() const noexcept -> float
		{
			return weis[0] + weis[1] + weis[2] + weis[3];
		}
		RTLIB_INLINE RTLIB_DEVICE      void AddWeiAtomic(int idx, float val) noexcept
		{
#ifdef __CUDA_ARCH__
			atomicAdd(&weis[idx], val);
#else
			weis[idx] += val;
#endif
		}
		...
		//RECORD
		RTLIB_INLINE RTLIB_DEVICE      void RecordInvDistance(float2& p, float value, float statisticalWeight, DTreeNode2* nodes) noexcept
		{
			DTreeNode2* cur = this;
			...
			cur->AddSumAtomic(idx, value);
			cur->AddWeiAtomic(idx, statisticalWeight);
			return;
		}
		//InvDistance
		RTLIB_INLINE RTLIB_DEVICE      auto InvDistance(float2& p, const DTreeNode2* nodes)const noexcept -> float
		{
			const DTreeNode2* cur = this;
			int idx = cur->GetChildIdx(p);
			int depth = 1;
			while (!cur->IsLeaf(idx))
			{
			...
			return cur->GetVal(idx);
		}
		//DEPTH
		RTLIB_INLINE RTLIB_HOST_DEVICE auto GetDepth(float2& p, const DTreeNode2* nodes) const noexcept -> int
		{
			const DTreeNode2* cur = this;
			int idx = cur->GetChildIdx(p);
			int depth = 1;
...
#ifndef __CUDA_ARCH__
		void Build(std::vector<DTreeNode2>& nodes)
		{
			for (int i = 0; i < 4; ++i)
			{
				if (this->IsLeaf(i))
		...
		void Dump(std::fstream& jsonFile, const std::vector<DTreeNode2>& nodes) const noexcept
		{
			jsonFile << "{\n";
	...
	struct DTree2
	{
		RTLIB_INLINE RTLIB_DEVICE      void AddWeiAtomic(float val) noexcept
		{
#ifdef __CUDA_ARCH__
			atomicAdd(&wei, val);
#else
		...
		RTLIB_INLINE RTLIB_DEVICE      void RecordInvDistance(float2 p, float invDistance, float statisticalWeight) noexcept
		{
			nodes[0].RecordInvDistance(p, invDistance, statisticalWeight, nodes);
		}
		RTLIB_INLINE RTLIB_DEVICE      auto InvDistance(float2 p)const noexcept -> float
		{
			return nodes[0].InvDistance(p, nodes);
		}
		DTreeNode2* nodes;
		float       sum;
		float       wei;
	};
	struct DTreeWrapper2
	{
		RTLIB_INLINE RTLIB_DEVICE void Record(const float3& direction, const float invDistance, const float statisticalWeight) noexcept
		{
			building.RecordInvDistance(rtlib::dir_to_canonical(direction), invDistance, statisticalWeight);
		}
		RTLIB_INLINE RTLIB_DEVICE auto InvDistance(const float3& direction)const noexcept -> float
		{
			return sampling.InvDistance(rtlib::dir_to_canonical(direction));
		}
		DTree2 building;
		DTree2 sampling;
	};
	...
	struct DTreeWrapper3
	{
		//forPg
		template <DirectionalFilter dFilter>
		RTLIB_INLINE RTLIB_DEVICE void Record(const DTreeRecord& rec) noexcept{ return forPg.Record<dFilter>(rec); }
		template <typename RNG>
		RTLIB_INLINE RTLIB_DEVICE auto Sample(RNG& rng)const noexcept -> float3{ return forPg.Sample(rng); }
		RTLIB_INLINE RTLIB_HOST_DEVICE auto Pdf(const float3& dir) const noexcept -> float
		{
			return forPg.Pdf(dir);
		}
		//forDs
		RTLIB_INLINE RTLIB_DEVICE void Record(const float3& direction, const float invDistance, const float statisticalWeight) noexcept
		{
			forDs.Record(direction, invDistance, statisticalWeight);
		}
		RTLIB_INLINE RTLIB_DEVICE auto InvDistance(const float3& direction)const noexcept -> float
		{
			return forDs.InvDistance(direction);
		}
	...
	struct STreeNode
	{
		...
		RTLIB_INLINE RTLIB_HOST_DEVICE auto GetDTreeWrapper(float3 &p, float3 &size, const STreeNode *nodes) const noexcept -> DTreeWrapper3*
		{
		...
		RTLIB_INLINE RTLIB_HOST_DEVICE auto GetDTreeWrapper() const noexcept -> DTreeWrapper3*
		{
			return dTree;
		}
		template <DirectionalFilter dFilter>
	...
	struct STree
	{
		RTLIB_INLINE RTLIB_HOST_DEVICE auto GetDTreeWrapper(float3 p, float3 &size) const noexcept -> DTreeWrapper3 *
		{
		...
		RTLIB_INLINE RTLIB_HOST_DEVICE auto GetDTreeWrapper(float3 p) const noexcept -> DTreeWrapper3 *
		{
			float3 size;
	...
	struct STreeNode2
	{
		...
		RTLIB_INLINE RTLIB_HOST_DEVICE auto GetDTreeWrapper(float3 &p, float3 &size, const STreeNode2 *nodes) const noexcept -> DTreeWrapper3 *
		{
