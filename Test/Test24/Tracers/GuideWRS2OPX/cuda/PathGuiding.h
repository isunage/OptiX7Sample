...
namespace test24_wrs2_guide
{
	...
	struct DTreeNode
	{
		...
		template <typename RNG>
		RTLIB_INLINE RTLIB_HOST_DEVICE auto SampleAndPdf(RNG& rng, const DTreeNode* nodes, float& pdf)const noexcept-> float2
		{
			const DTreeNode* cur = this;
			float2 result = make_float2(0.0f);
			pdf = 1.0f;
			...
			for (;;)
			{
				...
				if (cur->IsLeaf(idx) || cur->sums[idx] <= 0.0f)
				{
					auto sum_v = cur->GetSum(idx);
					if (sum_v > 0.0f && total > 0.0f) {
						pdf *= 4.0f * sum_v / total;
					}
					else {
						pdf = 0.0f;
					}
	...
	struct DTree
	{
		...
		template <typename RNG>
		RTLIB_INLINE RTLIB_HOST_DEVICE auto SampleAndPdf(RNG& rng, float& pdf) const noexcept -> float2
		{
			if (GetMean() <= 0.0f)
			{
				pdf = 1.0f / (4.0f * RTLIB_M_PI);
				return rtlib::random_float2(rng);
			}
			auto result = rtlib::clamp(nodes[0].SampleAndPdf(rng, nodes, pdf), make_float2(0.0f), make_float2(1.0f));
			pdf /= (4.0f * RTLIB_M_PI);
			return result;
		}
	...
	struct DTreeWrapper
	{
		...
		template <typename RNG>
		RTLIB_INLINE RTLIB_HOST_DEVICE auto SampleAndPdf(RNG& rng, float& pdf) const noexcept -> float3
		{
			return rtlib::canonical_to_dir(sampling.SampleAndPdf(rng,pdf));
		}
		DTree building;
		DTree sampling;
	};
	...
	struct TraceVertex
	{
		...
		RTLIB_INLINE RTLIB_HOST_DEVICE void Commit(STreeType& sTree, float statisticalWeight) noexcept
		{
			...
			/*float localRadianceAvg = rtlib::to_average_rgb(localRadiance);*/
			float localRadianceAvg = rtlib::to_average_rgb(localRadiance);
			/*float productAvg = rtlib::to_average_rgb(product);*/
			float productAvg = rtlib::to_average_rgb(product);
